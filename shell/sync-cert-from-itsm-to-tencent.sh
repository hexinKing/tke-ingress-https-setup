...
usage() {
    cat >&2 <<'EOF'
Usage: sync-cert-from-itsm-to-tencent.sh [options]

Required options:
  --project <name>           tccli profile 名
  --cert-list <file>         ITSM 证书域名清单文件

Optional:
  -h, --help                 显示本帮助

Environment variables:
  ITSM_CERT_GEN_WAIT_SECONDS 单个证书申请后的等待秒数，默认 70
EOF
}
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --cert-list)      itsm_cert_list_file="$2"; shift 2 ;;
        --cert-list=*)    itsm_cert_list_file="${1#*=}"; shift ;;
        -h|--help)        usage; exit 0 ;;
        *)
            echo "未知参数: $1" >&2
            usage
...
[[ -z "$itsm_cert_list_file" ]] && missing+=("--cert-list")
