...
完整发布链路：`sync-cert` → `create-ingress-and-clb`。
...
```bash
cd <workspace>/shell
./sync-cert-from-itsm-to-tencent.sh <project> <itsm_cert_list_file>
```

| 参数 | 含义 |
|---|---|
| `<project>` | 腾讯云 tccli 的 profile 名（对应 `~/.tccli/<project>.credential`） |
| `<itsm_cert_list_file>` | 待签发的 apex 域名清单文件路径 |
...
./sync-cert-from-itsm-to-tencent.sh adnetwork ./itsm.txt
```
...
# 3. 跑
./sync-cert-from-itsm-to-tencent.sh adnetwork /tmp/itsm-domains.txt
...
```bash
cd <workspace>/shell
./create-ingress-and-clb.sh <domain_list> <project> <context_region> <VPC_ID> <SUBNET_ID> <ns> <svc> <port> <REGION>
```

| 位置 | 参数 | 含义 | 示例 |
|---|---|---|---|
| `$1` | `domain_list` | 域名清单文件（支持 1 或 2 列）| `./domains.txt` |
| `$2` | `project` | tccli profile 名 | `adnetwork` |
| `$3` | `context_region` | k8s context 拼接用（`k8s-<region>-prod-<project>-tke-1`）| `vg` / `sg` |
| `$4` | `VPC_ID` | 腾讯云 VPC ID | `vpc-m7t7q9rf` |
| `$5` | `SUBNET_ID` | 腾讯云子网 ID | `subnet-rl7vqmvm` |
| `$6` | `ns` | k8s namespace | `default` |
| `$7` | `svc` | 后端 Service 名 | `my-app` |
| `$8` | `port` | 后端 Service 端口 | `80` |
| `$9` | `REGION` | 腾讯云 region | `na-ashburn` / `ap-singapore` |
...
# 没问题就真跑
./create-ingress-and-clb.sh \
    /tmp/new-domains.txt adnetwork vg vpc-m7t7q9rf subnet-rl7vqmvm default my-app 80 na-ashburn
